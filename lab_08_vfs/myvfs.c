...
#define MYVFS_MAGIC_NUMBER 0xAABBCCDD
#define SLAB_NAME "MyVFSCache"

static void **cache_mem_area;
...
static int my_vfs_fill_sb(struct super_block *sb, void *data, int silent)
{
    ...
    struct inode *root_inode = my_vfs_make_inode(sb, S_IFDIR | 0755);
    if (!root_inode)
    {
...
{
    printk(KERN_INFO "my_vfs_inode size = %zu\n", sizeof(struct my_vfs_inode));
